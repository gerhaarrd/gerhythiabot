...
class RhythiaSlashCommands(commands.Cog):
    ...
    async def verify(self, interaction: discord.Interaction) -> None:
        ...
        await interaction.response.defer(thinking=True, ephemeral=True)
        try:
            username = await verify_pending_rhythia_link(
                self.bot,
    ...
    async def beatmap(
        ...
        try:
            ...
            image_file = await _beatmap_image_file(beatmap)
            if image_file:
    ...
    async def recent(
        ...
        try:
            user_id, target_name = await self._resolve_user_for_scores(
                interaction.user.id,
                client,
                query=query,
            )
            ...
            embed = recent_score_embed(recent_score, username=target_name)
            ...
            if beatmap_hash:
                try:
                    ...
                    logger.warning("Failed to fetch beatmap for hash %s", beatmap_hash)

            image_file = await _beatmap_image_file(beatmap_obj) if beatmap_obj else None
            if image_file:
    ...
    async def _resolve_user_for_scores(
        self,
        discord_id: int,
...
async def _beatmap_image_file(beatmap: dict[str, Any] | None) -> discord.File | None:
    if beatmap is None:
        return None
